...
# 材料出库及时率
ECN_data = pd.read_excel(f"./KPI/SCM/OM/ECN单列表-{last_month_start}-{last_month_end}.XLSX",
                         usecols=['单据编号', '生产订单号', '审核日期'],
                         converters={'审核日期': datetime64})
# '母件编码', '母件名称', '旧子件编码',
# '旧子件名称', '旧子件基本用量', '新子件编码', '新子件名称', '新子件基本用量',
ECN_deal_data = pd.read_excel(f"./KPI/SCM/OM/ECN处理单列表-{last_month_start}-{last_month_end}.XLSX",

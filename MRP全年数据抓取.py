...
class OrderConversion:
    ...
    def GetOrderConversion(self):
        work_months = ['01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12']
        work_days = []
        for i in work_months:
            work_day = self.func.WorkDays2('2022', i)
            work_days.extend(work_day)
        # print(list_day)
        ...
        for work_day in work_days:
            if flag < 3:
                try:
                    base_data = pd.read_excel(f"{self.path}/DATA/SCM/OP/MRP计划维护--全部{work_day}.XLSX",
                    ...
                    catch_data = datetime64(f"{work_day}")
                ...
                try:
                    new_data = pd.read_excel(f"{self.path}/DATA/SCM/OP/MRP计划维护--全部{work_day}.XLSX",
                                             usecols=['物料编码', '物料名称', '需求跟踪号', '需求跟踪行号', '物料属性', '是否客供料'],
                                             converters={'物料编码': int, '需求跟踪行号': int}
                                             )
        ...
        for name1, group in all_mrp.groupby(['物料编码', '需求跟踪号', '需求跟踪行号', '抓取时间']):
        ...
        self.GroupMRPData.to_excel(f'./MRP全年数据抓取.xlsx', sheet_name="MRP清单", index=False)

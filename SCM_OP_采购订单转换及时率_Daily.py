...
class OrderConversion:
    def __init__(self):
        ...
        self.po_data = pd.read_excel(f"{self.path}/DATA/SCM/OP/采购订单列表.XLSX",
                                     usecols=['请购单号', '存货编码', '存货名称', '行号', '行关闭人'],
                                     converters={'请购单号': str, '订单编号': int, '行号': int, '存货编码': int}
                                     )
        self.po_data = self.po_data.loc[self.po_data["行关闭人"].isnull()]
        self.pr_data = pd.read_excel(f"{self.path}/DATA/SCM/OP/请购单列表.XLSX",
                                     usecols=['单据号', '存货编码', '存货名称', '行号', '行关闭人', '建议订货日期', '审核时间'],
                                     converters={'单据号': str, '行号': int, '存货编码': int, '审核时间': datetime64,
                                                 '建议订货日期': datetime64})
        self.pr_data = self.pr_data.loc[self.pr_data["行关闭人"].isnull()]
        self.pr_notapprove = self.pr_data.loc[self.pr_data["审核时间"].isnull()]
        self.pr_data = self.pr_data.loc[self.pr_data["审核时间"].isnotnull()]
        self.pr_notapprove['建议订货日期'] = datetime.date.today()
    ...
    def ThisMonthNotConverted(self):  # 当月未转换MRP清单
        ...
        self.pr_data.to_excel(f'{self.path}/RESULT/SCM/OP/采购订单转换及时率.xlsx', sheet_name="当月未转换采购订单的请购单", index=False)
        ...
        writer.book = book
        MRPNow_data.to_excel(writer, "当月未转换采购订单的MRP计划订单", index=False)
        writer.save()
    ...
    def HistoryNotConverted(self):  # 历史未转换MRP清单
        ...
        writer = pd.ExcelWriter(f"{self.path}/RESULT/SCM/OP/采购订单转换及时率.xlsx", engine='openpyxl')
        writer.book = book
        MRPHistory_data.to_excel(writer, "历史未转换采购订单的MRP计划订单", index=False)
        writer.save()

...
Material_data = pd.read_excel(f"./DATA/SCM/存货档案{_this_month_end}.XLSX",
...
this_month_merge_data.to_excel('./RESULT/SCM/OM/demo.xlsx', sheet_name="demo", index=False)

Routing_data = pd.read_excel(f"./DATA/SCM/OM/工艺路线资料表--含资源.xlsx",
...
BOM_data = pd.read_excel(f"./DATA/SCM/OM/BOM集成时间表.xlsx", header=1,
...
# 输出新旧物料及时率
new_Production_data.to_excel('./RESULT/SCM/OM/生产订单创建及时率.xlsx', sheet_name="新物料", index=False)
book = load_workbook('./RESULT/SCM/OM/生产订单创建及时率.xlsx')
writer = pd.ExcelWriter("./RESULT/SCM/OM/生产订单创建及时率.xlsx", engine='openpyxl')
writer.book = book

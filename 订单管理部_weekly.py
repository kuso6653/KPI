...
if __name__ == '__main__':
    ...
    work_days.extend(this_work_days)  # 将这个月上月工作日相合并
    ...
    for x in work_days:
        if flag < 3:
            try:  # 存货档案-20211001
                base_data = pd.read_excel(f"./KPI/SCM/OM/存货档案{year}-{this_month}-{x}.XLSX",
            ...
            try:
                now_data = pd.read_excel(f"./KPI/SCM/OM/存货档案{year}-{this_month}-{x}.XLSX",
                                         usecols=['存货编码', '存货名称', '计划默认属性', '固定提前期', '生产部门名称', '变动提前期', '变动基数'],
                                         converters={'最低供应量': int, '变动提前期': int, '变动基数': float}
                                         )

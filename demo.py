...
def CheckDataStock(first, two):
    ...
    out_data = pd.merge(first.drop(labels=['主要供货单位名称', '最低供应量', '采购员名称', '固定提前期', '计划默认属性', '启用日期'], axis=1), two,
                        on=['存货编码', '存货名称'])
...
if __name__ == '__main__':
    ...
    res.to_excel('./RESULT/SCM/SP/111.xlsx', sheet_name="当月大于7天采购物料维护及时率", index=False)

    book = load_workbook('./RESULT/SCM/SP/111.xlsx')
    writer = pd.ExcelWriter("./RESULT/SCM/SP/111.xlsx", engine='openpyxl')
    writer.book = book

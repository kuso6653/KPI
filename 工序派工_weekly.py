...
if __name__ == '__main__':
    ...
    work_days.extend(this_work_days)  # 将这个月工作和上个月合并
    ...
    for x in work_days:
        if flag < 3:
            try:
                base_data = pd.read_excel(f"./KPI/PROD/工序派工资料维护{year}-{this_month}-{x}.XLSX",
            ...
            try:
                now_data = pd.read_excel(f"./KPI/PROD/工序派工资料维护{year}-{this_month}-{x}.XLSX",
                                         usecols=['物料编码', '生产订单', '工序行号', '派工标识', '行号'],
                                         converters={'物料编码': int, '工序行号': int}
                                         )
            except:
